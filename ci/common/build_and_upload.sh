...
if [[ "${PROJECT_TYPE}" == "MAVEN" ]]; then
    ...
    if [[ "${CI}" == "CONCOURSE" ]]; then
        ./gradlew clean build deploy -x integrationTest -PnewVersion=${PIPELINE_VERSION} -DREPO_WITH_JARS=${REPO_WITH_JARS} --stacktrace  || ( $( printTestResults ) && return 1)
    else
        ./gradlew clean build deploy -x integrationTest -PnewVersion=${PIPELINE_VERSION} -DREPO_WITH_JARS=${REPO_WITH_JARS} --stacktrace
    fi
else
